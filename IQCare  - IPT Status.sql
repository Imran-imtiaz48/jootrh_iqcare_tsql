...
curr_treatmenttracker_cte AS (
	SELECT PatientId,Regimen,RegimenId, RegimenDate, Line as regimenLine FROM (
		SELECT RegimenId,ROW_NUMBER() OVER(PARTITION BY PatientId ORDER BY t.RegimenStartDate DESC, t.id DESC) as rowNum, t.PatientId,t.RegimenLine,t.Regimen,t.RegimenStartDate as RegimenDate, CASE WHEN t.RegimenLine LIKE '%First%' THEN '1' WHEN t.RegimenLine LIKE '%Second%' THEN '2' WHEN t.regimenLine LIKE '%third%' THEN 3 ELSE  NULL END as Line FROM PatientTreatmentTrackerViewD4T t WHERE t.Regimen IS NOT NULL AND YEAR(t.RegimenStartDate) >= 2000 AND t.RegimenStartDate <= @endDate
	) t WHERE t.rowNum = 1
),

init_treatmenttracker_cte AS (
	SELECT PatientId,Regimen,RegimenId, t.ARTInitiationDate, Line as regimenLine FROM (
		SELECT RegimenId, ROW_NUMBER() OVER(PARTITION BY PatientId ORDER BY t.RegimenStartDate ASC, t.RegimenStartDate ASC) as rowNum, t.PatientId,t.RegimenLine,t.Regimen, CAST(t.RegimenStartDate AS DATE) as ARTInitiationDate, CASE WHEN t.RegimenLine LIKE '%First%' THEN '1' WHEN t.RegimenLine LIKE '%Second%' THEN '2' WHEN t.regimenLine LIKE '%third%' THEN 3 ELSE  NULL END as Line FROM PatientTreatmentTrackerViewD4T t WHERE t.Regimen IS NOT NULL  AND YEAR(t.RegimenStartDate) >= 2000 AND t.RegimenStartDate <= @endDate --and t.RegimenStartDate IS NOT NULL
	) t WHERE t.rowNum = 1
),
...
ipt_status_cte AS (
	SELECT 
		PatientId, EverBeenOnIpt,IptStartDate,Completed,OnIpt,Discontinued,ReasonForDiscontinuation,rfd,CodedRFD,OutcomeDate
	FROM (
		SELECT 
			icf.PatientId
			,OutcomeDate
		...
		LEFT JOIN 
			(
				SELECT * FROM (
					SELECT PatientId,ReasonForDiscontinuation,IptDiscontinuationReason,IptEvent,CAST(ISNULL(IptOutcomeDate,CreateDate) AS DATE) as OutcomeDate, ROW_NUMBER() OVER(PARTITION BY PatientId ORDER BY CreateDate DESC) as RowNum FROM PatientIptOutcome 
				) o WHERE o.RowNum = 1
			)
...
/*
--ALL HIGH VL 
SELECT a.EnrollmentNumber as PatientId,a.PatientName,a.sex,a.PhoneNumber,a.ContactPhoneNumber,a.ContactName,a.currentAge, a.[EnrollmentDate ] as RegistrationDate,r.RegimenType as currentRegimen,a.NextAppointmentDate,v.LastHighVLDate as LastVLDate, v.LastHighVL as LastVL, sv.VlResultsDate as SecondLastVLDate,sv.VLResults as SecondLastVL, a.PatientStatus FROM all_Patients_cte a 
...
WHERE a.currentAge BETWEEN 10 and 19
...
-- select * from vitals_cte v 
/*
-- ALL PATIENTS
SELECT a.PatientId as Id,'JOT' as Facility, a.EnrollmentNumber as PatientId,ti.TINumber,mch.mchNumber,a.PatientName,a.sex, CASE WHEN a.PatientType = 258 THEN 'NEW' ELSE 'TI' END AS PatientType, '' as RegistrationAge,a.currentAge,  CAST(a.[EnrollmentDate] AS DATE) as RegistrationDate, ittx.Regimen as StartRegimen, CAST(art_init_date.ARTInitiationDate AS DATE) as ARTStartDate, cttx.Regimen as CurrentRegimen,cttx.regimenLine as CurrentRegimenLine,CAST(cttxdate.RegimenDate AS DATE) as CurrentRegimenStartdate,pttx.Regimen as PrevRegimen, pttx.regimenLine as PrevRegimenLine, CAST(pttxdate.RegimenDate AS DATE) as PrevRegimenStartDate, CAST(fv.firstVisitDate AS DATE) firstVisitDate, CAST(lv.lastVisitDate AS DATE) lastVisitDate,CAST(a.NextAppointmentDate AS DATE) NextAppointmentDate, CAST(vv.VlDate AS DATE) VlResultsDate, vv.VLResults, CAST(svv.VlResultsDate AS DATE) SecondLastVlResultsDate, svv.VLResults SecondLastVLResults, cd4.CD4Results,cd4.CD4Date, 
CASE WHEN a.PatientStatus = 'Death' THEN 'Dead' 
	WHEN a.PatientStatus = 'Transfer Out' THEN 'TransferOut' 
	WHEN DATEDIFF(D,a.NextAppointmentDate,@endDate) < 30 THEN 'Active' 
	WHEN DATEDIFF(D,a.NextAppointmentDate,@endDate) >= 30 AND DATEDIFF(D,a.NextAppointmentDate,@endDate) < 90 THEN 'Defaulter' 
...
WHERE EnrollmentDate <= @endDate -- AND a.PatientID = 6096
...
--atandi
SELECT a.PatientID as ID, a.EnrollmentNumber as [CCC Number],a.sex,CAST(a.DateOfBirth AS DATE) as DOB, a.CurrentAge as Age,
...
SELECT
	a.PatientId as Id, a.EnrollmentNumber as PatientId,a.PatientName,a.sex, CASE WHEN a.PatientType = 258 THEN 'NEW' ELSE 'TI' END AS PatientType,a.currentAge,  CAST(a.[EnrollmentDate] AS DATE) as RegistrationDate, ittx.Regimen as StartRegimen, CAST(art_init_date.ARTInitiationDate AS DATE) as ARTStartDate, cttx.Regimen as CurrentRegimen,
	ipt.EverBeenOnIpt,IptStartDate,Completed,OnIpt,Discontinued,ipt.ReasonForDiscontinuation,OutcomeDate,
	CASE WHEN a.PatientStatus = 'Death' THEN 'Dead' 
		WHEN a.PatientStatus = 'Transfer Out' THEN 'TransferOut' 
		WHEN DATEDIFF(D,a.NextAppointmentDate,@endDate) <= 30 THEN 'Active' 
--		WHEN DATEDIFF(D,a.NextAppointmentDate,@endDate) >= 30 AND DATEDIFF(D,a.NextAppointmentDate,@endDate) < 30 THEN 'Defaulter' 
		WHEN DATEDIFF(D,a.NextAppointmentDate,@endDate) >= 31 OR DATEDIFF(D,a.NextAppointmentDate,@endDate) IS NULL THEN 'LTFU' 
		ELSE a.PatientStatus 
END as PatientStatus
...
LEFT JOIN init_treatmenttracker_cte ittx ON ittx.PatientId = a.PatientID
LEFT JOIN curr_treatmenttracker_cte cttx ON cttx.PatientId = a.PatientID
LEFT JOIN ipt_status_cte ipt ON a.PatientId = ipt.patientId
WHERE a.EnrollmentDate <=@endDate
--WHERE PatientStatus = 'Active' AND
--DATEDIFF (D,a.NextAppointmentDate, @startDate) < 90
