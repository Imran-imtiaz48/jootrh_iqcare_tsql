...
-- WHERE g.PatientStatus = 'Death'
 ...
 baseline_cd4_results_cte AS (
	SELECT * FROM (
		SELECT *,ROW_NUMBER() OVER (Partition By PatientId ORDER BY CD4Date ASC) as RowNum FROM (
			SELECT        patientId,CAST(SampleDate AS DATE) as CD4Date, ResultValues  as CD4Results
			FROM            dbo.PatientLabTracker
			WHERE        (Results = 'Complete')
 ...
 cd4_results_cte AS (
	SELECT * FROM (
		SELECT        patientId,CAST(SampleDate AS DATE) as CD4Date, ResultValues  as CD4Results,ROW_NUMBER() OVER (Partition By PatientId ORDER BY SampleDate DESC) as RowNum
		FROM            dbo.PatientLabTracker
		WHERE        (Results = 'Complete')
		AND         (LabTestId = 1) AND SAmpleDate <= @endDate
	) cd4 WHERE RowNum = 1 
 ...
 cd4_percent_results_cte AS (
	SELECT * FROM (
		SELECT        patientId,CAST(SampleDate AS DATE) as CD4PercentDate, ResultValues  as CD4PercentResults,ROW_NUMBER() OVER (Partition By PatientId ORDER BY SampleDate DESC) as RowNum
		FROM            dbo.PatientLabTracker
		WHERE        (Results = 'Complete')
		AND         (LabName = 'CD4 Percent') AND SAmpleDate <= @endDate
	) cd4 WHERE RowNum = 1 
 ),

  peak_cd4_results_cte AS (
	SELECT * FROM (
		SELECT        patientId,CAST(SampleDate AS DATE) as CD4Date, ResultValues  as CD4Results,ROW_NUMBER() OVER (Partition By PatientId ORDER BY ResultValues DESC) as RowNum
		FROM            dbo.PatientLabTracker
		WHERE        (Results = 'Complete')
		AND         (LabTestId = 1) AND SAmpleDate <= @endDate
	) cd4 WHERE RowNum = 1 
 ...
 creatinine_results_cte AS (
	SELECT * FROM (
		SELECT        patientId,CAST(SampleDate AS DATE) as CreatinineDate, ResultValues  as CreatinineResults,ROW_NUMBER() OVER (Partition By PatientId ORDER BY SampleDate DESC) as RowNum
		FROM            dbo.PatientLabTracker
		WHERE        (Results = 'Complete')
		AND         (LabName = 'Creatinine') AND SAmpleDate <= @endDate
	) cd4 WHERE RowNum = 1 
 ),

  ast_results_cte AS (
	SELECT * FROM (
		SELECT        patientId,CAST(SampleDate AS DATE) as ASTDate, ResultValues  as ASTResults,ROW_NUMBER() OVER (Partition By PatientId ORDER BY SampleDate DESC) as RowNum
		FROM            dbo.PatientLabTracker
		WHERE        (Results = 'Complete')
		AND         (LabName = 'AST/SGOT') AND SAmpleDate <= @endDate
	) cd4 WHERE RowNum = 1 
 ),

  wbc_results_cte AS (
	SELECT * FROM (
		SELECT        patientId,CAST(SampleDate AS DATE) as WBCDate, ResultValues  as WBCResults,ROW_NUMBER() OVER (Partition By PatientId ORDER BY SampleDate DESC) as RowNum
		FROM            dbo.PatientLabTracker
		WHERE        (Results = 'Complete')
		AND         (LabName = 'WBC') AND SAmpleDate <= @endDate
	) cd4 WHERE RowNum = 1 
 ),

hb_results_cte AS (
	SELECT * FROM (
		SELECT        patientId,CAST(SampleDate AS DATE) as HBDate, ResultValues  as HBResults,ROW_NUMBER() OVER (Partition By PatientId ORDER BY SampleDate DESC) as RowNum
		FROM            dbo.PatientLabTracker
		WHERE        (Results = 'Complete')
		AND         (LabName = 'HB') AND SAmpleDate <= @endDate
	) cd4 WHERE RowNum = 1 
 ),

alt_results_cte AS (
	SELECT * FROM (
		SELECT        patientId,CAST(SampleDate AS DATE) as ALTDate, ResultValues  as ALTResults,ROW_NUMBER() OVER (Partition By PatientId ORDER BY SampleDate DESC) as RowNum
		FROM            dbo.PatientLabTracker
		WHERE        (Results = 'Complete')
		AND         (LabName = 'ALT/SGPT') AND SAmpleDate <= @endDate
	) cd4 WHERE RowNum = 1 
 ),
 ...
 peak_vl_cte AS (
	SELECT PatientId, VlDate, VLResults FROM (
		SELECT        patientId, VlDate, VLResults,ROW_NUMBER() OVER (Partition By PatientId ORDER BY VLResults DESC) as RowNum
											FROM            all_vl_cte v
	) r WHERE r.RowNum = 1
 ),
...
all_who_stage_cte AS (
	SELECT 
		w.PatientID,PatientMasterVisitId, l.Name AS WHOStage, ROW_NUMBER() OVER (PARTITION BY w.PatientId ORDER BY v.VisitDate DESC) as rown FROM PatientWHOStage w 
		INNER JOIN LookupItem AS l ON l.Id = w.WHOStage
		LEFT JOIN PatientMasterVisit v ON v.id = w.PatientMasterVisitId
	WHERE w.WHOStage != 500
),

latest_who_stage_cte AS (
		SELECT w.PatientId,w.PatientMasterVisitId, w.WHOStage FROM all_who_stage_cte w WHERE  rown = 1
),
...
-- KEMRI Perinatal study Data
SELECT
	/*a.PatientId as Id,*/ 
	a.EnrollmentNumber as PatientId,ti.TINumber,a.PatientName,a.sex, CASE WHEN a.PatientType = 258 THEN 'NEW' ELSE 'TI' END AS PatientType, DATEDIFF(YEAR, a.DateOfBirth, a.EnrollmentDate) as AgeAtRegistration,a.currentAge,  CAST(a.[EnrollmentDate] AS DATE) as RegistrationDate, 
	hd.HivDiagnosisDate, DATEDIFF(MONTH, a.DateOfBirth, hd.HivDiagnosisDate) AS  AgeAtHIVDiagnosis,bwho.WHOStage AS BaselineWHOStage,who.WHOStage AS LatestWHOStage,
	cd4.CD4Results AS LatestCD4,cd4.CD4Date as LatestCD4Date,
	pcd4.CD4Results AS PeakCd4, pcd4.CD4Date AS PeakCD4Date, 
	cd4pc.CD4PercentResults AS [CD4%Date], cd4pc.CD4PercentResults AS [CD4%Results],
	bvl.VLResults AS BaselineVL, bvl.VlDate AS BaselineVLDate, vl.VLResults AS LatestVL, vl.VlDate AS LatestVLDate,
	pvl.VLResults AS PeakVL, pvl.VlDate AS PeakVLDate,
	ittx.Regimen AS StartRegimen, CAST(art_init_date.ARTInitiationDate AS DATE) as ARTStartDate, ittx.regimenLine as InitialRegimenLine, 
	cttx.Regimen as CurrentRegimen,CAST(cttxdate.RegimenDate AS DATE) as CurrentRegimenStartdate, cttx.regimenLine AS CurrentRegimenLine,
	pttx.Regimen as PrevRegimen, CAST(pttxdate.RegimenDate AS DATE) as PrevRegimenStartDate, pttx.RegimenId AS PrevRegimenLine,
	wbc.WBCDate, wbc.WBCResults,
	hb.HBDate, hb.HBResults,
	ast.ASTDate, ast.ASTResults, 
	alt.ALTDate, alt.ALTResults, 
	ctn.CreatinineDate, ctn.CreatinineResults,
	CAST(lv.lastVisitDate AS DATE) lastVisitDate,CAST(a.NextAppointmentDate AS DATE) NextAppointmentDate, 
	cd4.CD4Results,cd4.CD4Date, a.PatientStatus,a.ExitDate
INTO #tmpPerinatal
FROM all_Patients_cte a 
LEFT JOIN patient_artintitiation_dates_cte art_init_date ON art_init_date.PatientId = a.PatientID
...
LEFT JOIN latest_who_stage_cte who ON who.PatientId = a.PatientID
LEFT JOIN cd4_percent_results_cte cd4pc ON cd4pc.patientId = a.PatientID
LEFT JOIN cd4_results_cte cd4 ON cd4.patientId = a.PatientID
LEFT JOIN baseline_vl_cte bvl ON bvl.patientId = a.PatientID
LEFT JOIN vl_cte vl ON vl.patientId = a.PatientID
LEFT JOIN peak_vl_cte pvl ON pvl.patientId = a.PatientID
LEFT JOIN peak_cd4_results_cte pcd4 ON pcd4.patientId = a.PatientID
LEFT JOIN init_treatmenttracker_cte ittx ON ittx.PatientId = a.PatientID
LEFT JOIN curr_treatmenttracker_cte cttx ON cttx.PatientId = a.PatientID
LEFT JOIN prev_treatmenttracker_cte pttx ON pttx.PatientId = a.PatientID
LEFT JOIN prev_regimen_date_cte pttxdate ON pttxdate.PatientId = a.PatientID
LEFT JOIN curr_regimen_date_cte cttxdate ON cttxdate.PatientId = a.PatientID
LEFT JOIN vl_cte vv ON vv.PatientId = a.PatientID
LEFT JOIN creatinine_results_cte ctn ON ctn.patientId = a.PatientID
LEFT JOIN alt_results_cte alt ON alt.patientId = a.PatientID
LEFT JOIN ast_results_cte ast ON ast.patientId = a.PatientID
--LEFT JOIN anc_results_cte anc ON anc.patientId = a.PatientID
LEFT JOIN wbc_results_cte wbc ON wbc.patientId = a.PatientID
LEFT JOIN hb_results_cte hb ON hb.patientId = a.PatientID
--LEFT JOIN second_last_vl_cte svv ON svv.PatientId = a.PatientID
LEFT JOIN last_visit_cte lv ON lv.PatientId = a.PatientID
--LEFT JOIN first_visit_cte fv ON fv.PatientId = a.PatientID
LEFT JOIN ti_cte ti ON ti.patientId = a.PatientID
...
-- Takes apprximately XX minutes to run


SELECT *FROM #tmpPerinatal p
