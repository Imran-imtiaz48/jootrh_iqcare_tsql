...
SELECT     g.Id as PatientID, g.PersonId, pc.MobileNumber as PhoneNumber,tp.ContactPhoneNumber,UPPER(tp.ContactName) AS ContactName, EnrollmentNumber, UPPER(tpm.PatientName) AS PatientName, CASE WHEN Sex = 52 THEN 'F' ELSE 'M' END AS Sex, DATEDIFF(M, [EnrollmentDate ], @endDate)/12 AS RegistrationAge, DATEDIFF(M, DateOfBirth, @endDate)/12 AS currentAge, '' AS EnrolledAt, CAST(CASE WHEN Ti.TransferInDate IS NOT NULL THEN ti.TransferInDate ELSE [EnrollmentDate ] END AS Date) as [EnrollmentDate] , '' as ARTStartDate, '' AS FirstVisitDate,'' AS LastVisitDate, P.NextAppointmentDate, 
...
LEFT JOIN (SELECT m.PatientPK, m.PatientName FROM [IQTools_KeHMIS].[dbo].[tmp_PatientMaster] m)tpm ON tpm.PatientPK = g.ptn_pk
 LEFT JOIN (
...
all_fp_pia_cte AS (
	SELECT 
		pia.PatientId,pia.PatientMasterVisitId,pr.PregnancyStatus,pr.LMP,pr.EDD,pr.[ANC/PNC],fp.CurrentlyOnFp,fp.ReasonNotOnFp,pia.ClientEligibleForFP,pia.PlanningToConceive3M,pia.ServiceForEligibleClient 
		,fpmethod = STUFF((
			  SELECT ',' + fpm.FpMethod
...
ipt_outcome_cte AS (
	SELECT PatientId, IptOutcome, ReasonForDiscontinuation,IptDiscontinuationReason FROM  (
		SELECT io.PatientId, li.Name AS IptOutcome,ReasonForDiscontinuation,IptDiscontinuationReason, ROW_NUMBER() OVER (PARTITION BY io.PatientId ORDER BY ISNULL(io.IptOutComeDate, io.CreateDate) DESC) AS rown FROM PatientIptOutcome io  LEFT JOIN PatientIptWorkup iw  ON iw.PatientId = io.PatientId 
		INNER JOIN LookupItem li ON li.Id = io.IptEvent
		WHERE IptDiscontinuationReason > 0 or len(ReasonForDiscontinuation) > 0
...
WHERE EnrollmentDate <= @endDate -- AND a.PatientID = 6166
