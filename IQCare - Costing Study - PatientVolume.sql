...
set @startDate ='2018-09-30';
set @endDate = @startDate ;-- '2017-10-31';
...
patient_population_cte AS (
	SELECT pp.PatientId, pp.PopulationType, pp.PopulationCategory FROM (
		SELECT  p.id AS PatientId, pp.PopulationType, l.Name AS PopulationCategory, ROW_NUMBER() OVER (PARTITION BY p.id ORDER BY pp.CreateDate DESC) rown 
		FROM PatientPopulation pp 
		INNER JOIN patient p 
			ON pp.PersonId = p.PersonId 
		LEFT JOIN LookupItem l 
			ON l.Id = pp.PopulationCategory
	) pp WHERE pp.rown = 1
),

service_area_cte AS (
	SELECT PatientId, ServiceArea FROM (
		SELECT 
			a.PatientId, a.PatientMasterVisitId, l.DisplayName AS ServiceArea, ROW_NUMBER() OVER(PARTITION BY a.PatientId ORDER BY a.AppointmentDate DESC)  rown
		FROM PatientAppointment a 
		INNER JOIN LookupItem l 
			ON a.ServiceAreaId = l.id
		INNER JOIN #tmpAllVisits v 
			ON a.PatientId = v.PatientId AND a.PatientMasterVisitId = v.PatientMasterVisitId
		WHERE 
			v.VisitDate <= @endDate 
	) sa WHERE sa.rown = 1
)
...
SELECT 
	DISTINCT 
	a.PatientId as Id,'JOT' as Site, 
	a.EnrollmentNumber as PatientId,
	CASE WHEN a.PatientType = 258 THEN 'NEW' ELSE 'TI' END AS [Patient Type], 
	a.currentAge AS [Current Age],
	CASE WHEN  a.currentAgeInMonths >= 24 AND a.currentAge <= 9 THEN 'Paediatric' ELSE CASE WHEN a.currentAge BETWEEN 9 AND 19 THEN 'Adolescent' ELSE CASE WHEN a.currentAge >= 20 THEN 'Adult' ELSE 'PMTCT Child' END END END AS AgeCategory,
	CAST(ittx.ARTInitiationDate AS DATE) as [ART Start Date], 
	s.EntryPoint AS EnrolledAT,
	vv.VLResults AS LastVL, 
	ISNULL(cat.Categorization, 'Unstable') AS Categorization,
	ISNULL(pp.PopulationType, 'General Population') AS PopulationType,
	pp.PopulationCategory,
	lv.LastProviderName,
	ipti.IPTStart,
	ipt.CompletedIpt,
	DATEDIFF(MONTH, COALESCE(startart.ARTInitiationDate, ittx.ARTInitiationDate), @endDate) AS DurationOnARTInMonths,
...
LEFT JOIN patientIptInitiations ipti ON ipti.PatientId = a.PatientID
LEFT JOIN service_entry_cte s ON s.PatientId = a.PatientID
LEFT JOIN patient_population_cte pp ON pp.PatientId = a.PatientID
LEFT JOIN service_area_cte sa ON sa.PatientId = a.PatientID
LEFT JOIN patient_artintitiation_dates_cte startart ON startart.PatientId = a.PatientID
