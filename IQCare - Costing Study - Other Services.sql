...
other_services_cte AS (
	SELECT * FROM nutrition_cte
)
...
SELECT 
	...
	s.currentAge AS [Age],
	CASE WHEN  s.AgeInMonths >= 24 AND s.currentAge <= 9 THEN 'Paediatric' ELSE CASE WHEN s.currentAge BETWEEN 9 AND 19 THEN 'Adolescent' ELSE CASE WHEN s.currentAge >= 20 THEN 'Adult' ELSE  'PMTCT Child' END END END AS AgeCategory,
	CAST(ittx.ARTInitiationDate AS DATE) as [ART Start Date], 
	ISNULL(sc.Categorization, 'Unstable') as Categorization,
	s.ServiceArea,
	s.OtherService,
	s.VisitDate,
	DATEDIFF(MONTH, ittx.ARTInitiationDate, s.VisitDate) AS DurationOnARTInMonths
/*for stans*/ 
FROM #tmpAllpatients a 
INNER JOIN other_services_cte s ON s.PatientId = a.PatientID
LEFT JOIN init_treatmenttracker_cte ittx ON ittx.PatientId = a.PatientID
LEFT JOIN all_stability_cte sc ON sc.PatientId = a.PatientID AND YEAR(sc.CategorizationDate) = YEAR(s.VisitDate) AND MONTH(sc.CategorizationDate) = MONTH(s.VisitDate)
