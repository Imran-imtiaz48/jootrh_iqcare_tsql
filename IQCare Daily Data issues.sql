...
LEFT JOIN (
 ...
 vl_results_cte AS (
	 SELECT        labTrac.patientId, PatientLabTracker_1.ResultValues, PatientLabTracker_1.FacilityId, CAST(PatientLabTracker_1.SampleDate AS DATE) AS SampleDate, CAST(PatientLabTracker_1.ResultDate AS DATE) AS ResultDate
	FROM            (SELECT      ROW_NUMBER() OVER (PARTITION BY PatientId ORDER BY SampleDate DESC) as rowNum, id, patientId
							  FROM            dbo.PatientLabTracker
							  WHERE        (Results = 'Complete')
							  AND         (LabTestId = 3)) AS labTrac INNER JOIN
							 dbo.PatientLabTracker AS PatientLabTracker_1 ON labTrac.Id = PatientLabTracker_1.Id WHERE labTrac.rowNum = 1
	-- WHERE        (PatientLabTracker_1.Results = 'Complete')
 ),

vl_cte AS (
	SELECT * FROM (
		SELECT        patientId,CAST(SampleDate AS DATE) as VlResultsDate, ResultValues  as VLResultsValue,ROW_NUMBER() OVER (Partition By PatientId ORDER BY SampleDate DESC) as RowNum
											FROM            dbo.PatientLabTracker
											WHERE        (Results = 'Complete')
...
second_last_vl_cte as (
 ...
 vitals_cte AS (
	SELECT VitalsId,PatientId,BPDiastolic,BPSystolic,BMI,VisitDate FROM (
		SELECT vi.Id as VitalsId, ROW_NUMBER() OVER(PARTITION BY vi.PatientId ORDER BY vi.CreateDate  DESC) as RowNUm, vi.CreateDate as VisitDate, vi.BPDiastolic,vi.BPSystolic,vi.BMI,vi.PatientId,vi.WeightForAge,vi.WeightForHeight,vi.BMIZ FROM PatientVitals vi INNER JOIN PatientMasterVisit pmv  ON vi.PatientId = pmv.PatientId 
	) v WHERE v.rowNUm =1
...
patient_artintitiation_dates_cte as (
	SELECT PatientId, CAST(min(ARTDate) AS DATE) as ARTInitiationDate FROM (
		...
		WHERE ptn_pharmacy_pk IN (SELECT ptn_pharmacy_pk FROM dbo.dtl_PatientPharmacyOrder o INNER JOIN mst_drug d ON d.drug_pk=o.drug_pk WHERE (Prophylaxis = 0) AND d.DrugName NOT LIKE '%COTRI%') AND o.DeleteFlag = 0 AND o.DispensedByDate IS NOT NULL
	) PatientARTdates
	GROUP BY patientId
),
...
treatmenttracker_cte AS (
	SELECT * FROM (
		SELECT ROW_NUMBER() OVER(PARTITION BY PatientId ORDER BY RegimenStartDate DESC) as rowNum, t.PatientId,t.RegimenLine,t.Regimen, CASE WHEN t.RegimenLine LIKE '%First%' THEN '1' WHEN t.RegimenLine LIKE '%Second%' THEN '2' WHEN t.regimenLine LIKE '%third%' THEN 3 ELSE  NULL END as Line FROM PatientTreatmentTrackerView t WHERE t.Regimen IS NOT NULL
	) t WHERE t.rowNum = 1
),

init_treatmenttracker_cte AS (
	SELECT * FROM (
		SELECT ROW_NUMBER() OVER(PARTITION BY PatientId ORDER BY RegimenStartDate ASC) as rowNum, t.PatientId,t.RegimenLine,t.Regimen, CAST(t.RegimenStartDate AS DATE) as ARTInitiationDate, CASE WHEN t.RegimenLine LIKE '%First%' THEN '1' WHEN t.RegimenLine LIKE '%Second%' THEN '2' WHEN t.regimenLine LIKE '%third%' THEN 3 ELSE  NULL END as Line FROM PatientTreatmentTrackerView t WHERE t.Regimen IS NOT NULL and t.RegimenStartDate IS NOT NULL
	) t WHERE t.rowNum = 1
),
...
location_cte AS (
	SELECT * FROM (
		SELECT PL.Id, PL.PersonId, PL.Location, C.CountyName, SC.Subcountyname, Patient.Id AS PatientId, W.WardName,ROW_NUMBER() OVER (Partition By Patient.Id ORDER BY PL.CreateDate DESC) as RowNum
...
completed_ipt_cte AS (
	SELECT PatientId, 'Y' as Completed FROM (
		SELECT icf.PatientId,ROW_NUMBER() OVER (PARTITION BY icf.PatientId ORDER BY icf.CreateDate DESC) as RowNum FROM PatientICF icf LEFT JOIN PatientIptOutcome ipt ON ipt.PatientId = icf.PatientId 
...
fp_cte AS (
	SELECT fpm.PatientFPId, l.DisplayName AS FPMethod FROM PatientFamilyPlanningMethod AS fpm INNER JOIN
								 LookupItem AS l ON fpm.FPMethodId = l.Id 
),

fp_method_cte AS (
		SELECT      fp.patientMasterVisitId, fp.PatientId,fp.VisitDate, 
					FpMethods = STUFF(
							(SELECT ',' + FPMethod FROM fp_cte AS fpm WHERE fp.Id = fpm.PatientFPId FOR XML PATH (''))
						,1,1,'')  
...
--WHERE v.ResultValues > 1000 AND v.ResultDate > '2017-01-01'
--WHERE RegimenType IS NOT NULL
*/
/*
--ALL VL 
SELECT a.EnrollmentNumber as PatientId,a.PatientName,a.sex,a.PhoneNumber,a.ContactPhoneNumber,a.ContactName,a.currentAge, a.[EnrollmentDate ] as RegistrationDate,r.RegimenType as currentRegimen,a.NextAppointmentDate,v.SampleDate as VLDate, v.ResultValues as VL,a.PatientStatus FROM all_Patients_cte a 
...
vl.SampleDate as VLDate, vl.ResultValues as VL,
...
a.[EnrollmentDate ] as RegistrationDate,
art.ARTInitiationDate as ARTStartDate,
vl.SampleDate as VLDate, vl.ResultValues as VL,/*ipt.IptDateCollected,CASE WHEN ipt.IptDateCollected IS NULL THEN 'N' ELSE 'Y' END as IptDone,*/
...
--select * from PatientTreatmentTrackerView WHERE PatientID = 827
--select * from treatmenttracker_cte WHERE PatientId= 827
--go
-- REGIMEN LINE
...
,vv.VlResultsDate,vv.VLResultsValue
,lv.lastVisitDate
...
FROM all_Patients_cte a 
LEFT JOIN init_treatmenttracker_cte art ON art.PatientId = a.PatientID
LEFT JOIN treatmenttracker_cte tt ON tt.PatientId = a.PatientID
...
--WHERE PatientStatus = 'Death' --AND PatientStatus !='Transfer Out'  --AND Regimen = 'Unknown'
...
--AND (a.ExitDate IS NULL OR a.ExitDate <'2018-05-01')
...
(SELECT CAST(MIN(ISNULL(ptt.RegimenStartDate,ptt.DispensedByDate)) AS DATE) FROM PatientTreatmentTrackerView ptt WHERE ptt.PatientID = a.PatientID and ptt.Regimen = art_cur.Regimen) 
as DateOFSwitchToCurrentRegimen, -- TODO
CAST(pr.LMP AS DATE) LMP,
CAST(pr.EDD AS DATE) EDD,
pr.PregnancyStatus as PregnancyStatusAtLastVisit,
vl.VlResultsDate as LastVLDate,
vl.VLResultsValue as LastVL,
vl2.VlResultsDate as SecondLastVLDate,
vl2.VLResultsValue as SecondLastVL,
a.PatientStatus,
...
INNER JOIN treatmenttracker_cte art_cur ON art_cur.PatientId = a.PatientID
