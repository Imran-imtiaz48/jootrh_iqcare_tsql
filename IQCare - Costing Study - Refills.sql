...
pharmacy_orders_cte AS (
	SELECT 
			DISTINCT
			p.id AS PatientId,
			CAST(o.OrderedbyDate AS DATE) AS PharmacyOrderDate,
			DATEDIFF(YEAR, p.DateOfBirth, o.OrderedbyDate) AS [CurrentAge],
			DATEDIFF(Month, p.DateOfBirth, o.OrderedbyDate) AS [AgeInMonths],
			o.PatientMasterVisitId,
			ROW_NUMBER() OVER (PARTITION BY p.id ORDER BY o.OrderedbyDate ASC) rown
	FROM 
		dtl_PatientPharmacyOrder pod
	INNER JOIN Mst_Drug d 
		ON pod.Drug_Pk = d.Drug_pk 
	INNER JOIN ord_PatientPharmacyOrder o
		ON pod.ptn_pharmacy_pk = o.ptn_pharmacy_pk
	INNER JOIN patient p
		ON p.ptn_pk = o.Ptn_Pk
	WHERE 
		d.Abbreviation IS NOT NULL
		AND CAST(o.OrderedbyDate AS DATE) BETWEEN @startDate AND @endDate
),

refills_cte AS (
	SELECT * FROM pharmacy_orders_cte o WHERE o.rown > 1
),
...
SELECT 
	...
	l.currentAge AS [Age],
	CASE WHEN  l.AgeInMonths >= 24 AND l.currentAge <= 9 THEN 'Paediatric' ELSE CASE WHEN l.currentAge BETWEEN 9 AND 19 THEN 'Adolescent' ELSE CASE WHEN l.currentAge >= 20 THEN 'Adult' ELSE  'PMTCT Child' END END END AS AgeCategory,
...
/*for stans*/ 
FROM #tmpAllpatients a 
INNER JOIN pharmacy_orders_cte l ON l.PatientId = a.PatientID
LEFT JOIN init_treatmenttracker_cte ittx ON ittx.PatientId = a.PatientID
LEFT JOIN all_stability_cte sc ON sc.PatientMasterVisitId = l.PatientMasterVisitId AND l.PatientID = sc.PatientId -- sc.PatientId = a.PatientID AND YEAR(sc.CategorizationDate) = YEAR(l.LabOrderDate) AND MONTH(sc.CategorizationDate) = MONTH(l.LabOrderDate)
LEFT JOIN service_area_cte sa ON sa.PatientId = l.PatientId AND sa.PatientMasterVisitId = l.PatientMasterVisitId
