...
all_visits_cte AS (
	SELECT PatientId,VisitDate,PatientMasterVisitId,ProviderId, ProviderName,UserId,GroupId FROM (
		SELECT PatientId,VisitDate,PatientMasterVisitId,ProviderId,p.ProviderName,P.GroupID,p.UserID,ROW_NUMBER() OVER (PARTITION BY PatientId,PatientMasterVisitId ORDER BY VisitDate DESC) as RowNum FROM (
			...
			UNION
				SELECT 
					p.id as PatientId, CAST(l.OrderedbyDate AS DATE) AS VisitDate,l1.PatientMasterVisitId, 100 AS LastProvider 
				FROM 
					ord_PatientLabOrder l 
				INNER JOIN Patient p 
					ON p.ptn_pk = l.Ptn_Pk
				INNER JOIN ord_LabOrder l1 
					ON l1.Id = l.LabID
				WHERE
					l.OrderedbyDate <= @endDate
			UNION
			SELECT p.id as PatientId, CAST(l.OrderedByDate AS DATE) AS VisitDate,l.PatientMasterVisitId, 100 AS LastProvider FROM ord_PatientPharmacyOrder l INNER JOIN Patient p ON p.ptn_pk = l.Ptn_Pk
			WHERE l.OrderedbyDate <= @endDate
...
/*
select * from (
select *, ROW_NUMBER() OVER(PARTITION BY PatientId ORDER BY VisitDate DESC) rn from #tmpAllVisits
) x WHERE x.rn = 1
return
*/
...
SELECT     g.Id as PatientID, g.ptn_pk as PatientPk, g.PersonId, pc.MobileNumber as PhoneNumber,
--tp.ContactPhoneNumber,UPPER(tp.ContactName) AS ContactName, 
EnrollmentNumber, 
--UPPER( tpm.PatientName) 
UPPER(CONCAT(g.FirstName, ' ', REPLACE(g.MiddleName, char(0),'') , ' ', g.LastName))
AS PatientName, 
CASE WHEN Sex = 52 THEN 'F' ELSE 'M' END AS Sex, DATEDIFF(M, [EnrollmentDate ], @endDate)/12 AS RegistrationAge, DATEDIFF(M, DateOfBirth, @endDate)/12 AS currentAge, DATEDIFF(M, DateOfBirth, @endDate) AS currentAgeInMonths, '' AS EnrolledAt, CAST(CASE WHEN Ti.TransferInDate IS NOT NULL THEN ti.TransferInDate ELSE [EnrollmentDate ] END AS Date) as [EnrollmentDate] , '' as ARTStartDate, '' AS FirstVisitDate,'' AS LastVisitDate, P.NextAppointmentDate, 
CASE WHEN ce.PatientId IS NULL THEN 'Active' ELSE ce.ExitReason END 
PatientStatus, CAST(ce.ExitDate AS DATE) as ExitDate, DateOfBirth, PatientType, 
--MaritalStatus, EducationLevel,
ce.ExitReason--, CareEndingNotes
...
SELECT * FROM (
	...
	AND IptStartDate < @endDate --BETWEEN @startDate AND @endDate
...
curr_month_dc_cte AS (
	SELECT PatientId, DCModel FROM (
		...
		INNER JOIN PatientMasterVisit AS V ON V.id = PA.PatientMasterVisitId
		WHERE V.VisitDate >= DATEADD(M, DATEDIFF(M, 0, @startDate), 0) and V.VisitDate <= EOMONTH(@startDate)
...
prev_month_dc_cte AS (
	SELECT PatientId, DCModel FROM (
		...
		FROM            PatientAppointment AS PA INNER JOIN
								 LookupItem AS L ON PA.DifferentiatedCareId = L.Id
		INNER JOIN PatientMasterVisit AS V ON V.id = PA.PatientMasterVisitId
		WHERE V.VisitDate >= DATEADD(M,DATEDIFF(M, 0, DATEADD(M, -1, @startDate)),0) and V.VisitDate <= EOMONTH(DATEADD(M, -1, @startDate))
	) dc WHERE dc.RowNum = 1
),
...
all_fp_pia_pr_summary_cte AS (
	...
	FROM (
		SELECT DISTINCT
		 ...
		 FROM #tmpAllpatients a 
			INNER JOIN #tmpAllVisits av ON av.PatientId = a.PatientID
...
all_who_stage_cte AS (
	SELECT DISTINCT v.PatientID, PatientMasterVisitId, REPLACE(l.Name,'Stage','') AS WHOStage,CAST(v.VisitDate AS DATE) AS VisitDate, l.DisplayName AS WHOStageName
	FROM PatientWHOStage w 
	INNER JOIN LookupItem l ON l.Id = w.WHOStage
	INNER JOIN PatientMasterVisit v ON v.id = w.PatientMasterVisitId
),

last_who_stage_cte AS (
	SELECT PatientId, WHOStage, WHOStageName, VisitDate FROM (
		SELECT *, ROW_NUMBER() OVER(PARTITION BY w.PatientId ORDER BY w.VisitDate DESC) rown FROM all_who_stage_cte w 
	) w WHERE w.rown = 1
),
...
ltfu_cte AS (
	SELECT *, DATEADD(D,31,t.AppointmentDate) tc FROM last_tca_cte t WHERE DATEADD(D,30,t.AppointmentDate) < @endDate AND DATEADD(D,30,t.AppointmentDate) >= @startDate -- DATEADD(D,-30,t.AppointmentDate) BETWEEN @startDate AND @endDate
),
...
service_entry_cte AS (
	SELECT * FROM (
		SELECT DISTINCT e.PatientId, l.Name AS EntryPoint, ROW_NUMBER() OVER (PARTITION BY e.PatientId ORDER BY e.CreateDate DESC) rown FROM ServiceEntryPoint e INNER JOIN LookupItem l ON e.EntryPointId = l.Id
	) s WHERE s.rown = 1
),

patient_population_cte AS (
	SELECT pp.PatientId, pp.PopulationType FROM (
		SELECT  p.id AS PatientId, pp.PopulationType, ROW_NUMBER() OVER (PARTITION BY p.id ORDER BY pp.CreateDate DESC) rown FROM PatientPopulation pp INNER JOIN patient p ON pp.PersonId = p.PersonId 
	) pp WHERE pp.rown = 1
),
...
appointments_cte AS (
		SELECT 
			...
			l.DisplayName AS ServiceArea, 
			CAST(a.AppointmentDate AS DATE) AS AppointmentDate, 
			CAST(COALESCE(v.VisitDate,v.CreateDate) AS DATE) AS VisitDate, 
			DATEDIFF(YEAR, p.DateOfBirth, v.VisitDate) AS [CurrentAge],
			DATEDIFF(Month, p.DateOfBirth, v.VisitDate) AS [AgeInMonths],
			ROW_NUMBER() OVER(PARTITION BY a.PatientId, a.AppointmentDate ORDER BY a.AppointmentDate DESC)  rown
		FROM PatientAppointment a 
		INNER JOIN LookupItem l ON a.ServiceAreaId = l.id
		INNER JOIN PatientMasterVisit v ON v.PatientId = a.PatientId AND a.PatientMasterVisitId = v.Id
...
nutrition_cte AS (
		SELECT 
			p.id AS PatientId, 
			'PSC Clinic' AS ServiceArea, 
			CAST(COALESCE(v.VisitDate,v.CreateDate) AS DATE) AS VisitDate, 
			DATEDIFF(YEAR, p.DateOfBirth, v.VisitDate) AS [CurrentAge],
			DATEDIFF(Month, p.DateOfBirth, v.VisitDate) AS [AgeInMonths],
			'Nutrition' AS OtherService,
			ROW_NUMBER() OVER(PARTITION BY p.id, v.VisitDate ORDER BY v.VisitDate DESC)  rown
		FROM DTL_FBCUSTOMFIELD_Nutrition_and_HIVAIDS_Register n 
		INNER JOIN ord_Visit v ON v.Ptn_Pk = n.Ptn_pk AND v.Visit_Id = n.Visit_Pk
		INNER JOIN Patient p ON p.ptn_pk = n.Ptn_pk
		WHERE v.VisitDate BETWEEN @startDate AND @endDate	
),

all_fp_cte AS (
		SELECT fp.id, ROW_NUMBER() OVER (Partition by fp.PatientId, fp.VisitDate Order By fp.VisitDate Desc) as rowNum, PatientId, CASE fp.FamilyPlanningStatusId WHEN  1 THEN 'Y' WHEN 2 THEN 'N' ELSE 'W' END AS CurrentlyOnFp, l.DisplayName as ReasonNotOnFp,VisitDate,PatientMasterVisitId FROM  
			PatientFamilyPlanning fp LEFT JOIN LookupItem l ON fp.ReasonNotOnFPId = l.Id 
		WHERE VisitDate BETWEEN @startDate AND @endDate	

),

clinical_visits_cte AS (
	SELECT 
		PatientId,
		...
		DATEDIFF(YEAR, p.DateOfBirth, v.VisitDate) AS [CurrentAge],
		DATEDIFF(Month, p.DateOfBirth, v.VisitDate) AS [AgeInMonths],
		ROW_NUMBER() OVER(PARTITION BY p.id, v.VisitDate ORDER BY v.VisitDate DESC, PatientMasterVisitId DESC)  rown
	FROM 
		#tmpAllVisits v
	INNER JOIN Patient p
		ON p.Id = v.PatientId
...
service_area_cte AS (
		SELECT 
			a.PatientId, a.PatientMasterVisitId, l.DisplayName AS ServiceArea, ROW_NUMBER() OVER(PARTITION BY a.PatientId ORDER BY a.CreateDate DESC)  rown
		FROM PatientAppointment a 
		INNER JOIN LookupItem l ON a.ServiceAreaId = l.id
)
...
SELECT 
	...
	c.currentAge AS [Age],
	CASE WHEN  c.AgeInMonths >= 24 AND c.currentAge <= 9 THEN 'Paediatric' ELSE CASE WHEN c.currentAge BETWEEN 9 AND 19 THEN 'Adolescent' ELSE CASE WHEN c.currentAge >= 20 THEN 'Adult' ELSE  'PMTCT Child' END END END AS AgeCategory,
	CAST(ittx.ARTInitiationDate AS DATE) as [ART Start Date], 
	ISNULL(sc.Categorization, 'Unstable') as Categorization,
	ISNULL(sa.ServiceArea, 'PSC Clinic') as ServiceArea,
	c.VisitDate,
	DATEDIFF(MONTH, COALESCE(startart.ARTInitiationDate, ittx.ARTInitiationDate), c.VisitDate) AS DurationOnARTInMonths
--	LastVl = (SELECT TOP 1 vl.VLResults FROM all_vl_cte vl WHERE vl.patientId = a.PatientID AND vl.VlDate<= c.VisitDate ORDER BY vl.VlDate DESC),
--	DurationOnIpt = DATEDIFF(MONTH, (SELECT TOP 1ipt.IPTStart FROM patientIptInitiations ipt WHERE ipt.PatientId = a.PatientID AND ipt.IPTStart <= c.VisitDate), c.VisitDate),
--	DurationOnCurrentRegimen = DATEDIFF(MONTH, cr.Regimendate, @endDate)
/*for stans*/ 
FROM #tmpAllpatients a 
INNER JOIN clinical_visits_cte c ON c.PatientId = a.PatientID
LEFT JOIN init_treatmenttracker_cte ittx ON ittx.PatientId = a.PatientID
--LEFT JOIN curr_regimen_date_cte cr On cr.PatientId = a.PatientID
LEFT JOIN patient_artintitiation_dates_cte startart On startart.PatientId = c.PatientId
LEFT JOIN all_stability_cte sc ON sc.PatientId = c.PatientId AND sc.PatientMasterVisitId = c.PatientMasterVisitId -- sc.PatientId = a.PatientID AND YEAR(sc.CategorizationDate) = YEAR(s.VisitDate) AND MONTH(sc.CategorizationDate) = MONTH(s.VisitDate)
LEFT JOIN service_area_cte sa ON sa.PatientId = c.PatientID AND sa.PatientMasterVisitId = c.PatientMasterVisitId
-- LEFT JOIN (SELECT * FROM all_vl_cte vl WHERE vl.patientId = a.PatientID) vl ON vl.patientId = a.PatientID and vl.VlDate <= c.VisitDate
WHERE a.EnrollmentDate <= @endDate 
AND ittx.Regimen IS NOT NULL
AND c.rown = 1
AND c.VisitDate >= COALESCE(startart.ARTInitiationDate,ittx.ARTInitiationDate)
-- AND a.PatientID = 244
