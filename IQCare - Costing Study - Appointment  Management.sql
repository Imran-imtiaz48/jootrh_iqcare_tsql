...
SELECT 
	...
	pa.currentAge AS [Age],
	CASE WHEN  pa.AgeInMonths >= 24 AND pa.currentAge <= 9 THEN 'Paediatric' ELSE CASE WHEN pa.currentAge BETWEEN 9 AND 19 THEN 'Adolescent' ELSE CASE WHEN pa.currentAge >= 20 THEN 'Adult' ELSE  'PMTCT Child' END END END AS AgeCategory,
	...
	pa.AppointmentDate,
	DATEDIFF(MONTH, ittx.ARTInitiationDate, pa.VisitDate) AS DurationOnARTInMonths
/*for stans*/ 
FROM #tmpAllpatients a 
INNER JOIN appointments_cte pa ON pa.PatientId = a.PatientID
LEFT JOIN init_treatmenttracker_cte ittx ON ittx.PatientId = a.PatientID
LEFT JOIN all_stability_cte sc ON sc.PatientMasterVisitId = pa.PatientMasterVisitId AND pa.PatientID = sc.PatientId -- sc.PatientId = a.PatientID AND YEAR(sc.CategorizationDate) = YEAR(l.LabOrderDate) AND MONTH(sc.CategorizationDate) = MONTH(l.LabOrderDate)
WHERE a.EnrollmentDate <= @endDate 
AND ittx.Regimen IS NOT NULL
AND  pa.VisitDate >= ittx.ARTInitiationDate 
AND pa.rown = 1
-- AND a.PatientID = 244
