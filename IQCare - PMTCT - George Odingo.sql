...
;WITH all_treatment_cte AS (
	SELECT * FROM (
		SELECT PatientMasterVisitId, RegimenId, ROW_NUMBER() OVER(PARTITION BY t.PatientId, PatientMasterVisitId ORDER BY t.RegimenStartDate DESC) as rowNum, t.PatientId,t.RegimenLine,t.Regimen,t.RegimenStartDate as RegimenDate,  CASE WHEN t.RegimenLine LIKE '%First%' THEN '1' WHEN t.RegimenLine LIKE '%Second%' THEN '2' WHEN t.regimenLine LIKE '%third%' THEN 3 ELSE  NULL END as Line,
...
FROM all_treatment_cte t
INNER JOIN mch_cte mch ON mch.PatientID = t.PatientId
WHERE mch.MCHEnrollmentDate BETWEEN @startDate AND @endDate
...
all_visits_cte AS (
	SELECT PatientId,VisitDate,PatientMasterVisitId,ProviderId, ProviderName,UserId,GroupId FROM (
		SELECT PatientId,VisitDate,PatientMasterVisitId,ProviderId,p.ProviderName,P.GroupID,p.UserID,ROW_NUMBER() OVER (PARTITION BY PatientId,PatientMasterVisitId ORDER BY VisitDate DESC) as RowNum FROM (
			SELECT v.PatientId,CAST(ISNULL(VisitDate,[Start]) AS DATE) AS VisitDate,v.Id as PatientMasterVisitId, v.CreatedBy as ProviderId FROM PatientMasterVisit v 
	...
	) visits WHERE RowNum = 1 
	-- AND visits.PatientId = 11926
...
;WITH vl_results_cte AS (
 ...
 cd4_results_cte AS (
  ...
  all_vl_cte AS (
	SELECT * FROM (
		SELECT ROW_NUMBER() OVER(PARTITION BY t.PatientId, t.PatientMAsterVisitId ORDER BY PatientMAsterVisitId) AS rown, patientId,t.PatientMasterVisitId,CAST(SampleDate AS DATE) as VlDate, CASE WHEN tr.Undetectable = 1  OR ResultTexts LIKE '%< LDL%' then 0 else ResultValues END  as VLResults
...
resuppression_cte AS (
 ...
 all_vitals_cte AS (
	SELECT PatientId, PatientMasterVisitId,Weight,Height,VisitDate,BMI,BPDiastolic,BPSystolic, Temperature, HeartRate FROM ( 
		SELECT  vi.PatientMasterVisitId,vi.Temperature, vi.HeartRate, vi.Id as VitalsId, ROW_NUMBER() OVER(PARTITION BY vi.PatientId, vi.PatientMasterVisitId ORDER BY vi.CreateDate  DESC) as RowNUm, ISNULL(pmv.VisitDate, pmv.start) AS VisitDate, vi.BPDiastolic,vi.BPSystolic,vi.BMI,vi.PatientId,vi.WeightForAge,vi.WeightForHeight,vi.BMIZ,vi.Weight,vi.Height 
...
last_visit_filtered_cte AS (
	SELECT visitDate as lastVisitDate, PatientId, PatientMasterVisitId, lastProvider,LastProviderName FROM (
		SELECT ROW_NUMBER() OVER (Partition by PatientId Order By visitDate Desc) as rowNum,PatientId,VisitDate,PatientMasterVisitId, v.UserId lastProvider, CONCAT(u.UserFirstName, ' ', u.UserLastName) as LastProviderName FROM #tmpAllVisits v INNER JOIN mst_User u ON v.UserId = u.UserID
		WHERE VisitDate <= @endDate AND VisitDate >= @startDate
	) lastVisit WHERE rowNum = 1  -- AND VisitDate < = @endDate
),
...
all_pregnancy_cte AS (
	SELECT * FROM (
		SELECT PatientID, PregnancyStatus,PregnancyOutcome, LMP, EDD,VisitDate,Outcome,DateOfOutcome,PatientMasterVisitId , [ANC/PNC],Parity, ROW_NUMBER() OVER(PARTITION BY PatientId,PatientMasterVisitId ORDER BY PatientMasterVisitId) rown FROM (
			SELECT        l.Name AS PregnancyOutcome, PI.Id, PI.PatientId, CAST(PI.LMP AS DATE) AS LMP, CASE WHEN L1.Name ='NPG' THEN NULL ELSE PI.EDD END AS EDD, CAST(PI.CreateDate AS DATE) as VisitDate, L1.DisplayName AS PregnancyStatus,P.Outcome,CAST(P.DateOfOutcome AS DATE) AS DateOfOutcome,pi.PatientMasterVisitId,CASE WHEN l1.Name = 'PG' THEN 'ANC' ELSE CASE pi.ANCProfile WHEN 1 THEN 'ANC' ELSE 'PNC' END END AS [ANC/PNC],Parity 
			FROM            PregnancyIndicator AS PI INNER JOIN LookupItem L1 ON PI.PregnancyStatusId = L1.Id
			LEFT JOIN Pregnancy P ON P.PatientId = PI.PatientId AND P.PatientMasterVisitId = PI.PatientMasterVisitId
			LEFT JOIN LookupItem l ON l.Id = p.Outcome
...
all_stability_cte AS (
	SELECT * FROM (
		SELECT ROW_NUMBER() OVER(PARTITION BY PatientId,PatientMasterVisitId ORDER BY CreateDate) as RowNum, s.PatientId, Case WHEN Categorization = 1 then 'Stable' ELSE 'Unstable' END as Categorization, PatientMasterVisitId, CAST(s.CreateDate AS DATE) as CategorizationDate from PatientCategorization s
...
all_pia_cte AS (
	SELECT * FROM (
		select ROW_NUMBER() OVER(PARTITION BY pia.PatientId, pia.PatientMasterVisitId ORDER BY pia.PatientMasterVisitId) rown, pia.PatientId,pia.VisitDate,pia.PlanningToConceive3M,pia.RegularMenses,pia.InitiatedOnART,pia.ClientEligibleForFP,l1.DisplayName as PartnerHIVStatus,l2.DisplayName as ServiceForEligibleClient,l3.DisplayName as ReasonForFPIneligibility,pia.patientMasterVisitId from PatientPregnancyIntentionAssessment pia 
...
all_fp_cte AS (
	SELECT PatientId, CurrentlyOnFp, ReasonNotOnFp, VisitDate, PatientMasterVisitId, id,
		...
		fpmethod = CASE WHEN CurrentlyOnFp = 'Y' THEN 
					STUFF((
						  SELECT ',' + fpm.FpMethod
					...
					ELSE NULL END
	  FROM (
		SELECT fp.id, ROW_NUMBER() OVER (Partition by fp.PatientId, fp.PatientMasterVisitId Order By fp.VisitDate Desc) as rowNum, PatientId, CASE fp.FamilyPlanningStatusId WHEN  1 THEN 'Y' WHEN 2 THEN 'N' ELSE 'W' END AS CurrentlyOnFp, l.DisplayName as ReasonNotOnFp,VisitDate,PatientMasterVisitId FROM  
...
all_who_stage_cte AS (
	SELECT * FROM (
		SELECT PatientID,PatientMasterVisitId, l.Name AS WHOStage, ROW_NUMBER() OVER(PARTITION BY w.PatientId, w.PatientMasterVisitId ORDER BY w.PatientMasterVisitId) rown FROM PatientWHOStage w INNER JOIN LookupItem l ON l.Id = w.WHOStage
	) w WHERE w.rown = 1
),
...
visits_count_cte AS (
	SELECT PatientId, count(*) as SumOfVisits FROM #tmpAllVisits GROUP BY PatientId 
),
...
-- select * from #tmpAllPatients
-- return 
-- select *from mch_cte WHERE YEAR(mch_cte.MCHEnrollmentDate) = 2018
-- return
...
SELECT 
	...
	att.Regimen AS RegimenAtVisit,
	ado.AdherenceOutcome,
	apr.PregnancyStatus,
	apr.LMP,
	apr.[ANC/PNC],
	CAST(CASE WHEN apr.[ANC/PNC] = 'ANC' THEN EDD ELSE NULL END AS DATE) as EDD,
	apr.pregnancyOutcome,
	CASE WHEN apr.PregnancyOutcome IS NOT NULL THEN DateOfOutcome ELSE NULL END AS DateOfPregnancyOutcome,
	apia.PlanningToConceive3M,
	afp.CurrentlyOnFp,
	afp.FpMethod,		
 ...
 FROM #tmpAllPatients a 
	...
	LEFT JOIN #tmpAllVisits av ON av.PatientId = a.PatientID
	...
	LEFT JOIN all_fp_cte afp ON afp.PatientMasterVisitId = av.PatientMasterVisitId
	LEFT JOIN all_pia_cte apia ON apia.PatientMasterVisitId = av.PatientMasterVisitId
	LEFT JOIN all_pregnancy_cte apr ON apr.PatientMasterVisitId = av.PatientMasterVisitId
	...
	LEFT JOIN all_vl_cte avl ON  avl.PatientMasterVisitId = av.PatientMasterVisitId
	LEFT JOIN #tmpAllTreatment att ON att.PatientMasterVisitId = av.PatientMasterVisitId
	LEFT JOIN all_who_stage_cte awho ON awho.PatientMastervisitId = av.PatientMasterVisitId
WHERE a.Sex = 'F'
AND YEAR(mch.MCHEnrollmentDate) = 2018 -- BETWEEN @startDate AND @endDate
AND av.PatientMasterVisitId > 0
-- AND DATEDIFF (D,a.NextAppointmentDate, @endDate) < 30
-- AND a.PatientID = 11427
ORDER BY av.PatientId, av.VisitDate
