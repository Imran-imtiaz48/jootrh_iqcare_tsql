...
--IF EXISTS(SELECT * FROM IQTOOLS_KEHMIS.sys.objects WHERE name = 'tmp_jot_201908' AND type='U')
--	DROP TABLE IQTOOLS_KEHMIS.dbo.tmp_jot_201908


IF EXISTS(SELECT * FROM IQTOOLS_KEHMIS.sys.objects WHERE name = 'leapWeek17' AND type='U')
	DROP TABLE IQTOOLS_KEHMIS.dbo.leapWeek17
...
--select * from stability_cte WHERE PatientId = 4
--return
--select * from #tmpAllpatients WHERE patientId = 10417
--return
--select * from gcPatientView WHERE id = 9878
--return
-- ALL PATIENTS
SELECT DISTINCT a.PatientId as Id,'JOT' as Site, a.EnrollmentNumber as PatientId,ti.TINumber AS [JOOT TI Number],mch.mchNumber AS [MCH Number],a.PatientName AS [Name],a.sex AS Sex, CASE WHEN a.PatientType = 258 THEN 'NEW' ELSE 'TI' END AS [Patient Type], '' as RegistrationAge,a.currentAge AS [Current Age],  CAST(a.[EnrollmentDate] AS DATE) as RegistrationDate, ittx.Regimen as StartRegimen,ittx.TLE400 as [Start regimen TLE400], CAST(ittx.ARTInitiationDate AS DATE) as [ART Start Date], cttx.Regimen as [Current regimen],cttx.TLE400 as [Current regimen TLE400],cttx.regimenLine as [Current regimen line],CAST(cttxdate.RegimenDate AS DATE) as [Current Regimen Start Date],pttx.Regimen as [Prev Regimen],pttx.TLE400 AS [Prev regimen TLE400], pttx.regimenLine as [Prev Regimen line], CAST(pttxdate.RegimenDate AS DATE) as [Prev Regimen Start Date], CAST(fv.firstVisitDate AS DATE) firstVisitDate, CAST(lv.lastVisitDate AS DATE) lastVisitDate,ctx.OnCTX, CASE WHEN tbs.PatientId IS NOT NULL THEN 'Y' ELSE 'N' END as [TB Screening],CASE WHEN tbs.TbScreeningOutcome = 'PrTB' THEN 'Y' ELSE 'N' END AS [Presumed TB],CAST(a.NextAppointmentDate AS DATE) NextAppointmentDate, CAST(stca.AppointmentDate AS DATE) PreviousNextAppoitmentDate, CAST(vv.VlDate AS DATE) AS LastVLDate, vv.VLResults AS LastVLValue, CAST(svv.VlResultsDate AS DATE) SecondLastVLDate, svv.VLResults AS SecondLastVLValue, cd4.CD4Results,cd4.CD4Date, 
FpMethod = Stuff((SELECT DISTINCT ',' + fp.FPMethod FROM fp_method_cte fp WHERE fp.PatientId = a.PatientID FOR XML PATH('')), 1, 1, ''),
ipt.CompletedIpt, 
...
cat.Categorization,
dc.DCModel,
vls.VlSampleDate AS LastVLSampleDate,
a.PhoneNumber,
a.ContactName,
a.ContactPhoneNumber,
who.WHOStage,
who.VisitDate AS WHODate,
...
[Outcome PEPFAR] = CASE WHEN a.PatientStatus = 'Death' THEN 'Dead' 
	WHEN a.PatientStatus = 'Transfer Out' THEN 'TransferOut' 
	WHEN DATEDIFF(D,a.NextAppointmentDate,@endDate) <= 30 THEN 'Active' 
	--WHEN DATEDIFF(D,a.NextAppointmentDate,@endDate) >= 30 AND DATEDIFF(D,a.NextAppointmentDate,@endDate) < 90 THEN 'Defaulter' 
	WHEN DATEDIFF(D,a.NextAppointmentDate,@endDate) > 30 OR DATEDIFF(D,a.NextAppointmentDate,@endDate) IS NULL THEN 'LostToFollowUp' 
	ELSE a.PatientStatus 
END,
[Outcome MOH]=CASE WHEN a.PatientStatus = 'Death' THEN 'Dead' 
	WHEN a.PatientStatus = 'Transfer Out' THEN 'TransferOut' 
	WHEN DATEDIFF(D,a.NextAppointmentDate,@endDate) <= 0 THEN 'Active' 
	WHEN DATEDIFF(D,a.NextAppointmentDate,@endDate) > 0 AND DATEDIFF(D,a.NextAppointmentDate,@endDate) < 90 THEN 'Defaulter' 
	WHEN DATEDIFF(D,a.NextAppointmentDate,@endDate) >= 90 OR DATEDIFF(D,a.NextAppointmentDate,@endDate) IS NULL THEN 'LostToFollowUp' 
	ELSE a.PatientStatus 
END
-- ,a.PhoneNumber,a.ContactPhoneNumber,a.ContactName
/*for stans*/ 
INTO IQTOOLS_KEHMIS.dbo.leapWeek17
FROM #tmpAllpatients a 
-- LEFT JOIN patient_artintitiation_dates_cte art_init_date ON art_init_date.PatientId = a.PatientID
LEFT JOIN init_treatmenttracker_cte ittx ON ittx.PatientId = a.PatientID
LEFT JOIN curr_treatmenttracker_cte cttx ON cttx.PatientId = a.PatientID
...
LEFT JOIN ctx_cte ctx ON ctx.PatientId = a.PatientID
LEFT JOIN last_who_stage_cte who ON who.PatientId = a.PatientId
LEFT JOIN secondlast_tca_cte stca ON stca.PatientId = a.PatientID
--WHERE EnrollmentDate <= @endDate 
-- AND a.PatientID = 244
...
select * from PatientTreatmentTrackerViewD4T WHERE PatientId = 34337 order by RegimenStartDate DESC


select * from ARVTreatmentTracker WHERE PatientId = 34337


DELETE from ARVTreatmentTracker WHERE PatientMasterVisitId = 188415

select * from ord_PatientPharmacyOrder WHERE PatientMasterVisitId = 188415

select * from dtl_RegimenMap WHERE Visit_Pk = 803256

select * from ord_PatientPharmacyOrder WHERE ptn_pharmacy_pk = 220099
