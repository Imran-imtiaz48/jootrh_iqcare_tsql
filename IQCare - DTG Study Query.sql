...
;WITH all_treatment_cte AS (
		SELECT * FROM (
			...
			TLE400 = (SELECT CASE WHEN COUNT(o.PatientMasterVisitId)>0 THEN 1 ELSE 0 END FROM dtl_PatientPharmacyOrder d
			 ...
			 FROM PatientTreatmentTrackerViewD4T t WHERE t.Regimen IS NOT NULL AND YEAR(t.RegimenStartDate) >= 2000 AND (t.RegimenStartDate <= @endDate AND t.RegimenStartDate >= @startDate)
		) t
)
...
dtg_start_cte AS (
	SELECT PatientId,Regimen,RegimenId, RegimenDate, Line as regimenLine FROM (
		SELECT RegimenId,ROW_NUMBER() OVER(PARTITION BY PatientId ORDER BY t.RegimenDate ASC) as rowNum, t.PatientId,t.RegimenLine,t.Regimen,t.RegimenDate as RegimenDate, CASE WHEN t.RegimenLine LIKE '%First%' THEN '1' WHEN t.RegimenLine LIKE '%Second%' THEN '2' WHEN t.regimenLine LIKE '%third%' THEN 3 ELSE  NULL END as Line
		FROM #tmpAllTreatment t WHERE t.Regimen LIKE '%DTG%' AND YEAR(t.RegimenDate) >= 2000 AND t.RegimenDate <= @endDate
	) t WHERE t.rowNum = 1	
),

dtg_end_cte AS (
	SELECT PatientId,Regimen,RegimenId, RegimenDate, Line as regimenLine FROM (
		SELECT RegimenId,ROW_NUMBER() OVER(PARTITION BY PatientId ORDER BY t.RegimenDate DESC) as rowNum, t.PatientId,t.RegimenLine,t.Regimen,t.RegimenDate as RegimenDate, CASE WHEN t.RegimenLine LIKE '%First%' THEN '1' WHEN t.RegimenLine LIKE '%Second%' THEN '2' WHEN t.regimenLine LIKE '%third%' THEN 3 ELSE  NULL END as Line
		FROM #tmpAllTreatment t WHERE t.Regimen LIKE '%DTG%' AND YEAR(t.RegimenDate) >= 2000 AND t.RegimenDate <= @endDate
	) t WHERE t.rowNum = 1		
),
...
efv_start_cte AS (
	SELECT PatientId,Regimen,RegimenId, RegimenDate, Line as regimenLine FROM (
		SELECT RegimenId,ROW_NUMBER() OVER(PARTITION BY PatientId ORDER BY t.RegimenDate ASC) as rowNum, t.PatientId,t.RegimenLine,t.Regimen,t.RegimenDate as RegimenDate, CASE WHEN t.RegimenLine LIKE '%First%' THEN '1' WHEN t.RegimenLine LIKE '%Second%' THEN '2' WHEN t.regimenLine LIKE '%third%' THEN 3 ELSE  NULL END as Line
		FROM #tmpAllTreatment t WHERE t.Regimen LIKE '%EFV%' AND YEAR(t.RegimenDate) >= 2000 AND t.RegimenDate <= @endDate
	) t WHERE t.rowNum = 1	
),
...
--- DTG AND Pregnancy
--- Victor RADIDA
SELECT a.EnrollmentNumber as CCCNumber,a.PatientName,a.DateOfBirth,a.Sex,cttx.Regimen, ds.RegimenDate as DTGStart,de.RegimenDate as DTGEnd, lmp.LMP AS LMPAtLastVisit, pg.LMP,pg.EDD,pg.Outcome,a.NextAppointmentDate,
	CASE
...
--- Victor RADIDA
SELECT a.EnrollmentNumber as CCCNumber,a.PatientName,a.DateOfBirth,a.Sex,cttx.Regimen, ds.RegimenDate as EFVStart,de.RegimenDate as EFVEnd, /*lmp.LMP*/ pg.LMP,pg.EDD,pg.Outcome,a.nextappointmentDate,
	CASE
		WHEN a.PatientStatus = 'Death' THEN 'Dead'
		WHEN a.PatientStatus = 'Transfer Out' THEN 'TransferOut'
		WHEN DateDiff(D, a.NextAppointmentDate, @endDate) <= 30 THEN 'Active'
