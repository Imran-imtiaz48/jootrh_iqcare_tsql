...
lab_tests_cte AS (
	SELECT 
		p.id AS PatientId,
		CAST(o.OrderedbyDate AS DATE) AS LabOrderDate,
		l.LabName AS LabName,
		o.VisitId,
		ol.PatientMasterVisitId,
		DATEDIFF(YEAR, p.DateOfBirth, o.OrderedbyDate) AS [CurrentAge],
		DATEDIFF(Month, p.DateOfBirth, o.OrderedbyDate) AS [AgeInMonths]
	FROM dtl_PatientLabResults pl 
	INNER JOIN ord_PatientLabOrder o 
		ON o.LabID = pl.LabID
	INNER JOIN ord_LabOrder ol 
		ON o.LabID = ol.Id
	INNER JOIN mst_LabTest l 
		ON pl.LabTestId = l.LabTestID
	INNER JOIN patient p
		ON p.ptn_pk = o.Ptn_Pk
	WHERE CAST(o.OrderedbyDate AS DATE) BETWEEN @startDate AND @endDate
),
...
/*for stans*/ 
FROM #tmpAllpatients a 
INNER JOIN lab_tests_cte l ON l.PatientId = a.PatientID
LEFT JOIN init_treatmenttracker_cte ittx ON ittx.PatientId = a.PatientID
