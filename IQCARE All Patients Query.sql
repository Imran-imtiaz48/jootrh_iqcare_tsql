...
curr_treatmenttracker_cte AS (
	SELECT PatientId,Regimen,RegimenId, RegimenDate, Line as regimenLine, TLE400 FROM (
		...
		TLE400
		 FROM all_treatmenttracker_cte t WHERE t.Regimen IS NOT NULL AND YEAR(t.RegimenDate) >= 2000 AND t.RegimenDate <= @endDate
...
init_treatmenttracker_cte AS (
	SELECT TLE400,PatientMasterVisitId, PatientId,Regimen,RegimenId, t.ARTInitiationDate, Line as regimenLine FROM (
		...
		TLE400		
		FROM all_treatmenttracker_cte t WHERE t.Regimen IS NOT NULL  AND YEAR(t.RegimenDate) >= 2000 AND t.RegimenDate <= @endDate --and t.RegimenStartDate IS NOT NULL
	) t WHERE t.rowNum = 1
),
...
last_visit_cte AS (
	SELECT lastVisitDate, PatientId, PatientMasterVisitId, lastProvider,LastProviderName FROM (
		SELECT ROW_NUMBER() OVER (Partition by PatientId Order By visitDate Desc) as rowNum, PatientId, v.VisitDate AS LastVisitdate, PatientMasterVisitId, v.ProviderId AS lastProvider, ProviderName as LastProviderName FROM all_visits_cte v
...
all_sti_cte AS (
	SELECT DISTINCT v.PatientId, PatientMasterVisitId, CreateBy AS CreatedBy, ISNULL(v.VisitDate, v.Start) AS VisitDate FROM PatientPHDP s
	INNER JOIN PatientMasterVisit v ON v.id = s.PatientMasterVisitId
	 WHERE Phdp = 76
),

sti_screening_cte AS (
	SELECT * from (
		SELECT ROW_NUMBER() OVER (Partition by PatientId Order By visitDate DESC) as rowNum, PatientId, VisitDate,PatientMasterVisitId, CreatedBy as lastProvider, StiScreening = 'Y' FROM all_sti_cte a   
	) ps WHERE ps.rowNum =1
...
dtg_ae_list_tce AS (
	SELECT ae.PatientMasterVisitId, ae.AdverseEventId, ae.PatientId, L.Name as AdverseEvent, ae.EventCause,ae.Action,CAST(v.VisitDate AS DATE) as EventDate
	-- ,curr.Regimen, pr.ProviderName 
	FROM AdverseEvent ae 
	INNER JOIN LookupItem l ON ae.AdverseEventId = l.Id
	-- INNER JOIN curr_treatmenttracker_cte curr ON curr.PatientId = ae.PatientId AND curr.PatientMasterVisitId = ae.PatientMasterVisitId
	-- INNER JOIN providers_cte pr ON pr.UserID = ae.CreateBy 
	--INNER JOIN patient p ON p.id = ae.PatientId
	-- INNER JOIN mst_Patient mp ON mp.Ptn_Pk = p.ptn_pk 
	INNER JOIN PatientMasterVisit v ON v.PatientId = ae.PatientId AND v.id = ae.PatientMasterVisitId
	WHERE v.VisitDate <= @EndDate
	AND (ae.EventCause LIKE '%DTG%' OR ae.EventCause LIKE '%dolut%')
...
all_tbscreening_cte AS (
	SELECT        PatientId, PatientMasterVisitId, CreatedBy as [Provider], CreateDate as VisitDate
	FROM            PatientScreening
	WHERE        (ScreeningTypeId = 4)
	UNION
	SELECT PatientId, PatientMasterVisitId, CreatedBy as [Provider], CreateDate as VisitDate FROM PatientIcf 
...
all_tb_icf_cte AS (
	SELECT PatientId, PatientMasterVisitId FROM PatientIcf 
),
...
menopausal_cte AS (
	SELECT PatientId,PatientMasterVisitId, ReasonNotOnFP FROM (
		SELECT PatientId,PatientMasterVisitId, ReasonNotOnFP, ROW_NUMBER() OVER (PARTITION BY PatientId ORDER BY PatientId) rown FROM (
			SELECT v.PatientId, PatientMasterVisitId, l.DisplayName AS ReasonNotOnFP FROM PatientPregnancyIntentionAssessment fpi
			INNER JOIN LookupItem l ON fpi.ReasonForFPIneligibility = l.Id
			INNER JOIN PatientMasterVisit v ON v.PatientId = fpi.PatientMasterVisitId
			UNION
			SELECT PatientId, PatientMasterVisitId, l.DisplayName AS ReasonNotOnFP FROM PatientFamilyPlanning fpi
			INNER JOIN LookupItem l ON fpi.ReasonNotOnFPId = l.Id
		) m WHERE m.ReasonNotOnFP = 'Menopausal'
	) m WHERE m.rown = 1
),
 ...
 optimized_vl_cte AS (
	SELECT PatientId, CASE WHEN ResultTexts LIKE '%LDL%' THEN 'LDL' ELSE CAST(ResultValues AS NVARCHAR) END AS Results FROM PatientLabTracker 
	WHERE LabName = 'Viral Load' AND Reasons= 'Single Drug Substitution' AND SampleDate BETWEEN '2019-04-01' AND '2019-04-30'
	 AND Results = 'Complete'

 )
 /*
 SELECT * FROM dtg_ae_list_tce
 RETURN
 */
 ...
 SELECT --a.PatientID as id,
	a.EnrollmentNumber as PatientId,ti.TINumber,a.PatientName,a.sex, a.currentAge, CAST(art_init_date.ARTInitiationDate AS DATE) as ARTStartDate, 
...
LEFT JOIN vl_cte vv ON vv.PatientId = a.PatientID
LEFT JOIN ti_cte ti ON ti.patientId = a.PatientID
INNER JOIN optimized_vl_cte op ON op.patientId = a.PatientID
WHERE EnrollmentDate <= GETDATE() --AND a.PatientID = 15
--AND DATEDIFF(D, a.NextAppointmentDate, GETDATE()) < 30
--AND CTTX.regimenLine = 3
--AND A.currentAge BETWEEN 0 AND 20
...
--AND (cttx.Regimen LIKE '%DRV%'
--OR cttx.Regimen LIKE '%TDF + 3TC + DTG%')
--AND vv.VLResults > 400
--AND DATEDIFF(M, vv.VlDate, @endDate) <= 6
RETURN
*/
...
-- select c.*, m.MaritalStatus from care_cte c INNER JOIN marital_status_cte m ON m.PatientId = c.Id
...
/*
-- Client TCAs
SELECT a.PatientID as ID, A.EnrollmentNumber AS PatientId, a.currentAge,a.NextAppointmentDate FROM all_Patients_cte a
*/
...
--- DTG AND Pregnancy
--- Victor RADIDA
SELECT a.EnrollmentNumber as CCCNumber,a.PatientName,a.DateOfBirth,a.Sex,cttx.Regimen, ds.RegimenDate as DTGStart,de.RegimenDate as DTGEnd, /*lmp.LMP*/ pg.LMP,pg.EDD,pg.Outcome,a.NextAppointmentDate FROM all_patients_cte a 
...
/*
--- EVER ON EFV, NEVER ON DTG 
--- Victor RADIDA
SELECT a.EnrollmentNumber as CCCNumber,a.PatientName,a.DateOfBirth,a.Sex,cttx.Regimen, ds.RegimenDate as EFVStart,de.RegimenDate as EFVEnd, /*lmp.LMP*/ pg.LMP,pg.EDD,pg.Outcome,a.nextappointmentf FROM all_patients_cte a 
...
LEFT JOIN vl_results_cte v ON a.PatientId = v.patientId
LEFT JOIN curr_treatmenttracker_cte r ON a.PatientID = r.PatientId
LEFT JOIN sti_screening_cte s ON r.PatientId =s.PatientId
-- LEFT JOIN relationship_cte rl ON rl.patientId = a.PatientID
WHERE v.VLResults >= 1000 AND v.VLDate >= @startDate
...
*/
/*
-- NON-OPTIMIZED CLIENTS - ADULT MALE ABOVE 15 NOT ON TLD
SELECT --a.PatientID as id,
	a.EnrollmentNumber as PatientId,ti.TINumber,a.PatientName,a.sex, a.currentAge, CAST(art_init_date.ARTInitiationDate AS DATE) as ARTStartDate, 
	REPLACE(CASE WHEN cttx.Regimen IS NULL THEN NULL ELSE SUBSTRING(cttx.Regimen,CHARINDEX('(',cttx.Regimen)+1,LEN(cttx.Regimen) - CHARINDEX('(',cttx.Regimen) - 1) END, ' + ', '/') as CurrentRegimen,
	CAST(a.NextAppointmentDate AS DATE) as AppointmentDate,  CAST(vv.VlDate AS DATE) LastVlDate, vv.VLResults as LastVL, 
	CASE WHEN a.PatientStatus = 'Death' THEN 'Dead' 
		...
		ELSE a.PatientStatus 
	END as PatientStatus,
	a.PhoneNumber, a.ContactName, a.ContactPhoneNumber, lv.LastProviderName --,  cttx.Regimen
FROM all_Patients_cte a 
LEFT JOIN patient_artintitiation_dates_cte art_init_date ON art_init_date.PatientId = a.PatientID
LEFT JOIN init_treatmenttracker_cte ittx ON ittx.PatientId = a.PatientID
LEFT JOIN curr_treatmenttracker_cte cttx ON cttx.PatientId = a.PatientID
...
AND CTTX.regimenLine = 1
AND (
	(A.currentAge >= 15 AND A.Sex ='M') 
--	OR (A.Sex='F' AND A.currentAge >= 50)
	)
AND a.PatientStatus = 'Active'
AND cttx.Regimen NOT LIKE '%DTG%'
AND cttx.Regimen NOT LIKE '%LPV/r%'
AND cttx.Regimen NOT LIKE '%ATV/r%'
AND vv.VLResults > 400
--AND DATEDIFF(M, vv.VlDate, @endDate) > 6
return
*/
/*
-- NON-OPTIMIZED CLIENTS FEMALE above 49 NOT ON DTG
SELECT --a.PatientID as id,
	a.EnrollmentNumber as PatientId,ti.TINumber,a.PatientName,a.sex, a.currentAge, CAST(art_init_date.ARTInitiationDate AS DATE) as ARTStartDate, 
	REPLACE(CASE WHEN cttx.Regimen IS NULL THEN NULL ELSE SUBSTRING(cttx.Regimen,CHARINDEX('(',cttx.Regimen)+1,LEN(cttx.Regimen) - CHARINDEX('(',cttx.Regimen) - 1) END, ' + ', '/') as CurrentRegimen,
	CAST(a.NextAppointmentDate AS DATE) as AppointmentDate,  CAST(vv.VlDate AS DATE) LastVlDate, vv.VLResults as LastVL, 
...
AND A.Sex ='F'
AND a.PatientStatus = 'Active'
AND (cttx.Regimen LIKE '%DTG%')
AND cttx.Regimen NOT LIKE '%LPV/r%'
AND cttx.Regimen NOT LIKE '%ATV/r%'
*/
/*
--xxxxxxxxxxxxxxxxxxxxxxxxxxxxx
-- OPTIMIZED CLIENTS - FEMALE REP AGE ON TLE
SELECT --a.PatientID as id,
	a.EnrollmentNumber as PatientId,ti.TINumber,a.PatientName,a.sex, a.currentAge, CAST(art_init_date.ARTInitiationDate AS DATE) as ARTStartDate, 
	REPLACE(CASE WHEN cttx.Regimen IS NULL THEN NULL ELSE SUBSTRING(cttx.Regimen,CHARINDEX('(',cttx.Regimen)+1,LEN(cttx.Regimen) - CHARINDEX('(',cttx.Regimen) - 1) END, ' + ', '/') as CurrentRegimen,
	CAST(a.NextAppointmentDate AS DATE) as AppointmentDate,  CAST(vv.VlDate AS DATE) LastVlDate, vv.VLResults as LastVL, 
...
AND A.Sex ='F'
AND a.PatientStatus = 'Active'
AND (cttx.Regimen LIKE '%TDF + 3TC + EFV%'
OR cttx.Regimen LIKE '%TDF + 3TC + DTG%')
--AND vv.VLResults <= 400
AND DATEDIFF(M, vv.VlDate, @endDate) <= 6
UNION
-- OPTIMIZED CLIENTS - ADULT MALE ABOVE 15 and FEMALE above 49 NOT ON TLD
SELECT --a.PatientID as id,
	a.EnrollmentNumber as PatientId,ti.TINumber,a.PatientName,a.sex, a.currentAge, CAST(art_init_date.ARTInitiationDate AS DATE) as ARTStartDate, 
	REPLACE(CASE WHEN cttx.Regimen IS NULL THEN NULL ELSE SUBSTRING(cttx.Regimen,CHARINDEX('(',cttx.Regimen)+1,LEN(cttx.Regimen) - CHARINDEX('(',cttx.Regimen) - 1) END, ' + ', '/') as CurrentRegimen,
	CAST(a.NextAppointmentDate AS DATE) as AppointmentDate,  CAST(vv.VlDate AS DATE) LastVlDate, vv.VLResults as LastVL, 
...
lv.LastProviderName
FROM all_Patients_cte a 
INNER JOIN gc_last_visit_cte lv ON lv.PatientId = a.PatientID
LEFT JOIN ti_cte ti ON ti.PatientId = a.PatientID
LEFT JOIN mch_cte mch ON mch.PatientID = a.PatientID 
...
/* 
--LTFU - Lost to folow up
SELECT 
	a.EnrollmentNumber AS CCCNumber, 
	a.PatientName,
	a.Sex,
	a.currentAge,
	CASE WHEN a.PatientStatus = 'Death' THEN 'Dead' 
		WHEN a.PatientStatus = 'Transfer Out' THEN 'TransferOut' 
		WHEN DATEDIFF(D,l.AppointmentDate,@endDate) < 30 THEN 'Active' 
		WHEN DATEDIFF(D,l.AppointmentDate,@endDate) >= 30 AND DATEDIFF(D,l.AppointmentDate,@endDate) < 90 THEN 'Defaulter' 
		WHEN DATEDIFF(D,l.AppointmentDate,@endDate) >= 90 OR DATEDIFF(D,l.AppointmentDate,@endDate) IS NULL THEN 'LTFU' 
		ELSE a.PatientStatus 
	END as PatientStatus,
	l.AppointmentDate,
	v.LastProviderName,
...
/*
-- Women of Reproductive Age(WRA) ON DTG and and on non-effective FP methods
SELECT 
	a.PatientID as ID, a.EnrollmentNumber as PatientId, a.PatientName,a.currentAge as Age,a.PhoneNumber, a.ContactName,a.ContactPhoneNumber, art.Regimen as CurrentRegimen,
	FpMethod = (Stuff((SELECT DISTINCT ',' + fp.FPMethod FROM fp_method_cte fp WHERE fp.PatientId = a.PatientID FOR XML PATH('')), 1, 1, '')), a.NextAppointmentDate,
	CASE WHEN a.PatientStatus = 'Death' THEN 'Dead' 
		WHEN a.PatientStatus = 'Transfer Out' THEN 'TransferOut' 
		WHEN DATEDIFF(D,a.NextAppointmentDate,@endDate) < 30 THEN 'Active' 
		WHEN DATEDIFF(D,a.NextAppointmentDate,@endDate) >= 30 AND DATEDIFF(D,a.NextAppointmentDate,@endDate) < 90 THEN 'Defaulter' 
...
INNER JOIN curr_treatmenttracker_cte art ON art.PatientId = a.PatientID 
WHERE 
	a.currentAge BETWEEN 15 AND 49 AND
	a.Sex = 'F' AND
	art.Regimen LIKE '%DTG%' AND
	(
		(
			(Stuff((SELECT DISTINCT ',' + fp.FPMethod FROM fp_method_cte fp WHERE fp.PatientId = a.PatientID FOR XML PATH('')), 1, 1, '')) NOT LIKE '%Implant%' AND
			(Stuff((SELECT DISTINCT ',' + fp.FPMethod FROM fp_method_cte fp WHERE fp.PatientId = a.PatientID FOR XML PATH('')), 1, 1, '')) NOT LIKE '%Intra Uterine Device%' AND
			(Stuff((SELECT DISTINCT ',' + fp.FPMethod FROM fp_method_cte fp WHERE fp.PatientId = a.PatientID FOR XML PATH('')), 1, 1, '')) NOT LIKE '%Tubal Ligation%'
		)
		OR
		(Stuff((SELECT DISTINCT ',' + fp.FPMethod FROM fp_method_cte fp WHERE fp.PatientId = a.PatientID FOR XML PATH('')), 1, 1, '')) IS NULL
	)
...
SELECT 
	/*a.PatientId as Id,*/ a.EnrollmentNumber as PatientId,CASE a.sex WHEN 'F' THEN 'Female' ELSE 'Male' END as Sex, CAST(a.DateOfBirth AS DATE) AS DateOfBirth, hd.HivDiagnosisDate, CAST(a.EnrollmentDate AS DATE) AS EnrollmentDate,
	-- SUBSTRING(bl.WHOStage, PATINDEX('%[0-9]%',bl.WHOStage),1) AS WHOStage, 
	CAST(art_init_date.ARTInitiationDate AS DATE) as ARTStartDate, 
	REPLACE(CASE WHEN ittx.Regimen IS NULL THEN NULL ELSE SUBSTRING(ittx.Regimen,CHARINDEX('(',ittx.Regimen)+1,LEN(ittx.Regimen) - CHARINDEX('(',ittx.Regimen) - 1) END, ' + ', '/') as StartRegimen, 
	REPLACE(CASE WHEN cttx.Regimen IS NULL THEN NULL ELSE SUBSTRING(cttx.Regimen,CHARINDEX('(',cttx.Regimen)+1,LEN(cttx.Regimen) - CHARINDEX('(',cttx.Regimen) - 1) END, ' + ', '/') as CurrentRegimen,
	vit.BMI, CASE WHEN tbs.PatientId IS NOT NULL THEN 'Yes' ELSE 'No' END AS TbScreening, tbs.TbScreeningOutcome,
    ipt.IptStartDate, ipt.IptOutcome, ipt.outcomedate, svv.VLResults SecondlastVL, svv.VlResultsDate SecondLastVLDate, vv.VLResults MostRecentVL, vv.VlDate MostRecentVLDate, lv.lastVisitDate, a.NextAppointmentDate,CASE WHEN pia.PregnancyStatus = 'PG' THEN 'Already pregnant' WHEN pia.PlanningToConceive3M IS NOT NULL THEN 'Yes' ELSE 'No' END,
	'NA' AS EIDWIthin8W, 'NA' AS InfantProphylaxis,'NA' AS NoVisitsFile, vcount.SumOfVisits AS VisitsEMR, lv.LastProviderName
...
-- MENOPAUSAL ON DTG
SELECT a.PatientID AS ID, a.EnrollmentNumber AS CCCNumber, a.PatientName, a.currentAge as Age, t.Regimen, m.ReasonNotOnFP 
FROM all_Patients_cte a 
	INNER JOIN curr_treatmenttracker_cte t ON a.PatientId = t.PatientId
	INNER JOIN menopausal_cte m ON m.PatientId = t.PatientId
WHERE t.Regimen LIKE '%DTG%'
return

SELECT 
	a.PatientID, a.EnrollmentNumber, a.PatientName, v.VisitDate,
	CASE WHEN a.PatientStatus = 'Death' THEN 'Dead' 
		WHEN a.PatientStatus = 'Transfer Out' THEN 'TransferOut' 
		WHEN DATEDIFF(D,a.NextAppointmentDate,@endDate) < 30 THEN 'Active' 
		WHEN DATEDIFF(D,a.NextAppointmentDate,@endDate) >= 30 AND DATEDIFF(D,a.NextAppointmentDate,@endDate) < 90 THEN 'Defaulter' 
...
FROM #tmpV v 
WHERE
  DATEDIFF(M, (SELECT TOP 1 VisitDate FROM #tmpV pvv WHERE pvv.PatientId = v.PatientId AND pvv.VisitDate < v.VisitDate ORDER BY pvv.VisitDate DESC), v.VisitDate) = 3
--AND v.patientId = 834
GROUP BY PatientId
HAVING COUNT(*) >= 2
*/
