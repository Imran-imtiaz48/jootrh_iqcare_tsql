...
;WITH vl_results_cte AS (
 ...
 cd4_results_cte AS (
  ...
  all_vl_cte AS (
	SELECT        DISTINCT patientId,CAST(SampleDate AS DATE) as VlDate, CASE WHEN tr.Undetectable = 1  OR ResultTexts LIKE '%< LDL%' then 0 else ResultValues END  as VLResults, t.PatientMasterVisitId
...
viremics_cte AS (
	SELECT 
			vl.patientId,
			vl.PatientMasterVisitId,
			vl.VlDate,
			vl.VLResults,
			DATEDIFF(YEAR, p.DateOfBirth, vl.VlDate) AS [CurrentAge],
			DATEDIFF(Month, p.DateOfBirth, vl.VlDate) AS [AgeInMonths],
			CAST(COALESCE(v.VisitDate,v.CreateDate) AS DATE) AS VisitDate
	FROM 
		all_vl_cte vl
	INNER JOIN Patient p 
		ON p.id = vl.PatientId
	INNER JOIN PatientMasterVisit v 
		ON v.PatientId = vl.PatientId -- AND vl.PatientMasterVisitId = v.Id
	WHERE 
		vl.VlDate BETWEEN @startDate AND @endDate
		AND vl.VLResults >= 1000
)
...
SELECT 
	...
	vl.currentAge AS [Age],
	CASE WHEN  vl.AgeInMonths >= 24 AND vl.currentAge <= 9 THEN 'Paediatric' ELSE CASE WHEN vl.currentAge BETWEEN 9 AND 19 THEN 'Adolescent' ELSE CASE WHEN vl.currentAge >= 20 THEN 'Adult' ELSE  'PMTCT Child' END END END AS AgeCategory,
	CAST(COALESCE(ittx.ARTInitiationDate, artstart.ARTInitiationDate) AS DATE) as [ART Start Date], 
	ISNULL(sc.Categorization, 'Unstable') as Categorization,
	ISNULL(pa.ServiceArea, 'PSC Clinic') AS ServiceArea,
	vl.VLDate,
	vl.VLResults,
	DATEDIFF(MONTH, COALESCE(ittx.ARTInitiationDate, artstart.ARTInitiationDate), vl.VLDate) AS DurationOnARTInMonths
/*for stans*/ 
FROM #tmpAllpatients a 
INNER JOIN viremics_cte vl ON vl.patientId = a.PatientID
LEFT JOIN service_area_cte pa ON pa.PatientId = vl.PatientID AND pa.PatientMasterVisitId = vl.PatientMasterVisitId
LEFT JOIN init_treatmenttracker_cte ittx ON ittx.PatientId = a.PatientID
LEFT JOIN patient_artintitiation_dates_cte artstart ON artstart.PatientId = a.PatientID
LEFT JOIN all_stability_cte sc ON sc.PatientMasterVisitId = pa.PatientMasterVisitId AND pa.PatientID = sc.PatientId 
--WHERE COALESCE(ittx.ARTInitiationDate, artstart.ARTInitiationDate) IS NOT NULL
WHERE a.EnrollmentDate <= @endDate 
AND COALESCE(ittx.ARTInitiationDate, artstart.ARTInitiationDate) IS NOT NULL
--AND  vl.VisitDate >= ittx.ARTInitiationDate 
--AND pa.rown = 1
-- AND a.PatientID = 244
